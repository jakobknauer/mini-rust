...
pub type MlrBuilderResult<T> = Result<T, MlrBuilderError>;

#[derive(Debug)]
pub enum MlrBuilderError {
    TyErr(typechecker::TyError),
}

impl From<typechecker::TyError> for MlrBuilderError {
    fn from(val: typechecker::TyError) -> Self {
        MlrBuilderError::TyErr(val)
    }
}
...
impl<'a> MlrBuilder<'a> {
    ...
    fn insert_val(&mut self, val_def: mlr::ValDef) -> MlrBuilderResult<mlr::Val> {
        let val = self.ctxt.mlr.insert_val(val_def);
        self.typechecker().infer_val_ty(val)?;
        Ok(val)
    }

    pub fn insert_call_val(&mut self, callable: mlr::Op, args: Vec<mlr::Op>) -> MlrBuilderResult<mlr::Val> {
        let val = mlr::ValDef::Call { callable, args };
        self.insert_val(val)
    }

    pub fn insert_use_val(&mut self, op: mlr::Op) -> MlrBuilderResult<mlr::Val> {
        let val = mlr::ValDef::Use(op);
        self.insert_val(val)
    }

    pub fn insert_use_place_val(&mut self, place: mlr::Place) -> MlrBuilderResult<mlr::Val> {
        let op = self.insert_copy_op(place)?;
        self.insert_use_val(op)
    }

    pub fn insert_addr_of_val(&mut self, place: mlr::Place) -> MlrBuilderResult<mlr::Val> {
        let val = mlr::ValDef::AddrOf(place);
        self.insert_val(val)
    }

    pub fn insert_as_val(&mut self, op: mlr::Op, target_ty: ty::Ty) -> MlrBuilderResult<mlr::Val> {
        let val_def = mlr::ValDef::As { op, target_ty };
        self.insert_val(val_def)
    }

    pub fn insert_size_of_val(&mut self, ty: ty::Ty) -> MlrBuilderResult<mlr::Val> {
        let val_def = mlr::ValDef::SizeOf(ty);
        self.insert_val(val_def)
    }

    pub fn insert_ptr_offset_val(&mut self, ptr: mlr::Op, offset_op: mlr::Op) -> MlrBuilderResult<mlr::Val> {
        let val = mlr::ValDef::PtrOffset(ptr, offset_op);
        self.insert_val(val)
    }

    fn insert_stmt(&mut self, stmt_def: mlr::StmtDef) -> MlrBuilderResult<mlr::Stmt> {
    ...
    pub fn insert_if_stmt(&mut self, cond: mlr::Op, then: mlr::Stmt, else_: mlr::Stmt) -> MlrBuilderResult<mlr::Stmt> {
        let if_ = mlr::StmtDef::If(mlr::If { cond, then, else_ });
        self.insert_stmt(if_)
    }

    pub fn insert_loop_stmt(&mut self, body: mlr::Stmt) -> MlrBuilderResult<mlr::Stmt> {
        let val = mlr::StmtDef::Loop { body };
        self.insert_stmt(val)
    }

    pub fn insert_alloc_stmt(&mut self, loc: mlr::Loc) -> MlrBuilderResult<mlr::Stmt> {
        let stmt = mlr::StmtDef::Alloc { loc };
        self.insert_stmt(stmt)
    }

    pub fn insert_fresh_alloc(&mut self) -> MlrBuilderResult<mlr::Place> {
        let undef = self.ctxt.tys.undef_ty();
        self.insert_alloc_with_ty(undef)
    }

    pub fn insert_alloc_with_ty(&mut self, ty: ty::Ty) -> MlrBuilderResult<mlr::Place> {
        let loc = self.ctxt.mlr.insert_typed_loc(ty);
        self.insert_alloc_stmt(loc)?;
        self.insert_loc_place(loc)
    }

    pub fn insert_break_stmt(&mut self) -> MlrBuilderResult<mlr::Stmt> {
        let stmt = mlr::StmtDef::Break;
        self.insert_stmt(stmt)
    }

    pub fn insert_assign_stmt(&mut self, place: mlr::Place, value: mlr::Val) -> MlrBuilderResult<mlr::Stmt> {
        let stmt = mlr::StmtDef::Assign { place, value };
        self.insert_stmt(stmt)
    }

    pub fn insert_assign_to_loc_stmt(&mut self, loc: mlr::Loc, value: mlr::Val) -> MlrBuilderResult<mlr::Stmt> {
        let place = mlr::PlaceDef::Loc(loc);
        let place = self.insert_place(place)?;
        self.insert_assign_stmt(place, value)
    }

    pub fn insert_return_stmt(&mut self, value: mlr::Val) -> MlrBuilderResult<mlr::Stmt> {
        let stmt = mlr::StmtDef::Return { value };
        self.insert_stmt(stmt)
    }

    fn insert_place(&mut self, place_def: mlr::PlaceDef) -> MlrBuilderResult<mlr::Place> {
        let place = self.ctxt.mlr.insert_place(place_def);
        self.typechecker().infer_place_ty(place)?;
        Ok(place)
    }

    pub fn insert_loc_place(&mut self, loc: mlr::Loc) -> MlrBuilderResult<mlr::Place> {
        let place = mlr::PlaceDef::Loc(loc);
        self.insert_place(place)
    }

    pub fn insert_enum_discriminant_place(&mut self, base: mlr::Place) -> MlrBuilderResult<mlr::Place> {
    ...
    pub fn insert_project_to_variant_place(
        ...
        base: mlr::Place,
        variant_index: usize,
    ) -> MlrBuilderResult<mlr::Place> {
        let place = mlr::PlaceDef::ProjectToVariant { base, variant_index };
        self.insert_place(place)
    }

    pub fn insert_field_access_place(&mut self, base: mlr::Place, field_index: usize) -> MlrBuilderResult<mlr::Place> {
        let place = mlr::PlaceDef::FieldAccess { base, field_index };
        self.insert_place(place)
    }

    pub fn insert_deref_place(&mut self, op: mlr::Op) -> MlrBuilderResult<mlr::Place> {
        let place = mlr::PlaceDef::Deref(op);
        self.insert_place(place)
    }

    pub fn insert_closure_captures_place(&mut self, closure_place: mlr::Place) -> MlrBuilderResult<mlr::Place> {
        let place = mlr::PlaceDef::ClosureCaptures(closure_place);
        self.insert_place(place)
    }

    fn insert_op(&mut self, op_def: mlr::OpDef) -> MlrBuilderResult<mlr::Op> {
        let op = self.ctxt.mlr.insert_op(op_def);
        self.typechecker().infer_op_ty(op)?;
        Ok(op)
    }

    pub fn insert_fn_op(&mut self, fn_: fns::Fn) -> MlrBuilderResult<mlr::Op> {
    ...
    pub fn insert_fn_inst_op(&mut self, fn_inst: fns::FnInst) -> MlrBuilderResult<mlr::Op> {
        self.ctxt.fns.register_fn_call(self.target_fn, fn_inst);
        let op = mlr::OpDef::Fn(fn_inst);
        self.insert_op(op)
    }

    pub fn insert_trait_mthd_op(&mut self, trait_mthd_inst: fns::TraitMthdInst) -> MlrBuilderResult<mlr::Op> {
        self.ctxt.fns.register_trait_mthd_call(self.target_fn, trait_mthd_inst);
        let op = mlr::OpDef::TraitMthd(trait_mthd_inst);
        self.insert_op(op)
    }

    pub fn insert_int_op(&mut self, int: i64) -> MlrBuilderResult<mlr::Op> {
        let val = mlr::OpDef::Const(mlr::Const::Int(int));
        self.insert_op(val)
    }

    pub fn insert_bool_op(&mut self, boolean: bool) -> MlrBuilderResult<mlr::Op> {
        let op = mlr::OpDef::Const(mlr::Const::Bool(boolean));
        self.insert_op(op)
    }

    pub fn insert_unit_op(&mut self) -> MlrBuilderResult<mlr::Op> {
    ...
    pub fn insert_c_char_op(&mut self, c_char: u8) -> MlrBuilderResult<mlr::Op> {
        let op = mlr::OpDef::Const(mlr::Const::CChar(c_char));
        self.insert_op(op)
    }

    pub fn insert_c_string_op(&mut self, c_string: &[u8]) -> MlrBuilderResult<mlr::Op> {
        let op = mlr::OpDef::Const(mlr::Const::CString(c_string.to_vec()));
        self.insert_op(op)
    }

    pub fn insert_copy_op(&mut self, place: mlr::Place) -> MlrBuilderResult<mlr::Op> {
        let op = mlr::OpDef::Copy(place);
        self.insert_op(op)
    }

    pub fn insert_typed_loc(&mut self, ty: ty::Ty) -> MlrBuilderResult<mlr::Loc> {
        let loc = self.ctxt.mlr.insert_typed_loc(ty);
        Ok(loc)
    }
