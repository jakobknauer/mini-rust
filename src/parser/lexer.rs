use crate::parser::token::{Keyword, Token};
...
impl<'a> Lexer<'a> {
    ...
    fn try_parse_identifier_or_keyword(&mut self) -> Option<Token> {
        ...
        let identifier = &self
            ...
            .collect::<String>();
        let token = to_keyword_token(identifier).unwrap_or_else(|| Token::Identifier(identifier.to_string()));
...
fn to_keyword_token(keyword: &str) -> Option<Token> {
    match keyword {
...
mod test {
    ...
    fn test_ident_with_digits() {
        ...
        let input = "var_123";
        let tokens = get_tokens(input);
        assert_eq!(tokens, vec![super::Token::Identifier(input.to_string())]);
    }
