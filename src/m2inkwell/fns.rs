...
pub struct M2InkwellFn<'a, 'iw, 'mr> {
    gtor: &'a mut super::M2Inkwell<'iw, 'mr>,
...
impl<'a, 'iw, 'mr> M2InkwellFn<'a, 'iw, 'mr> {
    pub fn new(gtor: &'a mut super::M2Inkwell<'iw, 'mr>, specialization: mr_fns::FnSpecialization) -> Option<Self> {
    ...
    pub fn define_fn(&mut self) -> M2InkwellFnResult<()> {
    ...
    fn build_alloca(&mut self, iw_ty: BasicTypeEnum<'iw>, name: &str) -> M2InkwellFnResult<PointerValue<'iw>> {
        ...
        let current_block = self.builder.get_insert_block().ok_or(M2InkwellFnError)?;
