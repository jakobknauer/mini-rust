...
fn try_find_instantiation_internal<'ty>(
    ...
    match (target.0, generic.0) {
        ...
        (&Ref(inner1), &Ref(inner2))
        | (&RefMut(inner1), &RefMut(inner2))
        | (&Ptr(inner1), &Ptr(inner2))
        | (&Slice(inner1), &Slice(inner2)) => try_find_instantiation_internal(inner1, inner2, instantiation),
