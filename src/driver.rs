...
impl<'a, 'arena> Driver<'a, 'arena> {
    ...
    fn populate_language_item_traits(&mut self) -> Result<(), DriverError<'arena>> {
        ...
        self.ctxt.language_items.bit_xor_trait = Some(self.ctxt.traits.resolve_trait_name("BitXor").ok_or_else(err)?);
        ...
        self.ctxt.language_items.index_trait = Some(self.ctxt.traits.resolve_trait_name("Index").ok_or_else(err)?);
