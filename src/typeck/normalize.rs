...
impl<'a, 'ctxt: 'a> super::Typeck<'a, 'ctxt> {
    ...
    pub(super) fn normalize(&mut self, ty: ty::Ty<'ctxt>) -> ty::Ty<'ctxt> {
        ...
        match *ty.0 {
            ...
            Opaque { opaque, gen_args } => {
                ...
                self.ctxt.tys.inst_opaque_from_ty_slice(opaque, gen_args).unwrap()
            }
    ...
    fn normalize_expr_extra(&mut self, extra: ExprExtra<'ctxt>) -> ExprExtra<'ctxt> {
        match extra {
            ...
            ExprExtra::UnaryOpMthd(resolution) => ExprExtra::UnaryOpMthd(self.normalize_mthd_resolution(resolution)),
